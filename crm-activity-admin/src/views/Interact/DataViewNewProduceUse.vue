...
export default {
    ...
    return {
     ...
     initPrizeRecordsList: [
        ...
        },
        {
          ...
          render: (h, params) => {
            // 只有一张图片时
            if (params.row.Img1 && !params.row.Img2) {
              return h("div", [
                h("a", {
                  domProps: {
                    innerHTML:
            ...
            // 有两张图片时
            else if (params.row.Img2 && !params.row.Img3) {
              return h("div", [
                h("a", {
                  domProps: {
                    innerHTML:
  ...
  methods: {
    ...
    GetAllPrizeRecords() {
      ...
      this.$post("/api/crmNewProductApply/GetListBySC", data).then(res => {
        if (res.state == 1) {
          this.pRList =[];
          if(res.rows.length){
            let temp=[]
            Object.keys(res.rows[0].msgList).forEach(i=>{
            ...
            this.PrizeRecordsList=[...this.initPrizeRecordsList,...temp]
            res.rows.forEach(i=>{
              this.pRList.push({...i,...i.msgList})
            })
          }
    ...
    SubmitDelData(name) {
      (this.$refs[name].model.activeNo = this.ActNo),
        this.$post(
          "/api/crmNewProductApply/DeleteTestData",
