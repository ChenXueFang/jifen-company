...
Page({
  ...
  onLoad: function(options) {
    ...
    // 爸爸妈妈能签到
    var myRole =  wx.getStorageSync("myrole").UserRole;
    if (myRole == "Mather" || myRole == "Father"){
    ...
    // 家人进来数据为空，不能操作
      ...
      this.setData({
        noSignedDays:[], //漏签数据为空
        signedDays: [], //签到数据为空
        signImg: "grayClickSignin.png", //灰色‘点击签到’
        medalImg1: this.data.img[3], //家人进来，默认为全灰的孕妈勋章
  ...
  judgeClickImg() {
    // 今天已签，当前月，显示“您已签到”
    if (this.data.todaySign && this.data.yearChange == this.data.year && this.data.monthChange == this.data.month) {
      this.setData({
        signImg: "alreadySign.png", //‘您已签到’
      })
    }
    // 今天没有签到，默认选中今天；显示黄色‘点击签到’
    if (!this.data.todaySign && this.data.yearChange == this.data.year && this.data.monthChange == this.data.month) {
      ...
      this.setData({
        signImg: "clickSignin.png", //黄色‘点击签到’
        demo6_days_style: demo6_days_style,
      })
    }
    // 切换到其他月，非当前月，显示灰色‘点击签到’
    if (this.data.yearChange != this.data.year || this.data.monthChange != this.data.month) {
      this.setData({
        signImg: "grayClickSignin.png", //灰色‘点击签到’
      })
    }
  ...
  getSignInfo: async function() {
    ...
    var hr = await signApi.Sign.getSignInfo({
      ...
      Remark: `${this.data.yearChange}-${this.data.monthChange}-01`,
    });
    if (hr.state == 1) {
  ...
  getUserSign: async function() {
    ...
    if (hr.state == 1) {
      ...
      // 今天已签到
      if (hr.rows.length>0){
        if (hr.rows[0].SignDate.substring(0, 10) == this.getcurrentdate(1)) {
          this.setData({
      ...
      if (this.data.yearChange == new Date().getFullYear() && this.data.monthChange == new Date().getMonth() + 1) {
        ...
        // 本月已签到天数，历史记录
        this.setData({
          signNumCurrent: signedDays.length,
        })
      }
      this.setData({
        signedDays: signedDays,
        noSignedDays: noSignedDays,
        signNum: signedDays.length, //签到天数
        noSignNum: noSignedDays.length, //漏签天数
      })
    } else {
      this.setData({
        signedDays: [],
  ...
  clickSign: async function() {
    ...
    var myRole = this.data.myRole; //角色缓存
    ...
    // 家人进入，显示灰色点击签到，点击提示只有爸爸和妈妈能进行签到
    else if (this.data.signImg == "grayClickSignin.png") {
      if (myRole != "Mather" && myRole != "Father"){
        wx:wx.showToast({
          title: '当前仅支持孕爸/孕妈累计幸运星',
          icon: 'none',
        })
      }
    }
