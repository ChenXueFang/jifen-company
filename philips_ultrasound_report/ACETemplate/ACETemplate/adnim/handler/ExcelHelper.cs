...
namespace ACETemplate.adnim.handler 
{
    public class ExcelHelper : IDisposable
    {
        ...
        public DataTable ExcelToDataTable(string sheetName, int rowIndex, bool isFirstRowColumn)
        {
            ISheet sheet = null;
            ...
            try
            {
                ...
                if (sheet != null)
                {
                    ...
                    for (int i = startRow; i <= rowCount; ++i)
                    {
                        ...
                        if (row == null || row.PhysicalNumberOfCells == 0) continue; //没有数据的行默认是null　　　　　　　
                            ...
                            if (tempcell != null) //同理，没有数据的单元格都默认是null
                            {
                                switch (tempcell.CellType)
                                {
                                    ...
                                    case CellType.String:
                                        ...
                                        var t = "\u200C" + tempcell.StringCellValue;
                                        break;
                                    ...
                                    case CellType.Formula:
                                        if (tempcell != null)
                                        {
                                            ...
                                            if(tempcell.StringCellValue!=null)
                                            {
                                                dataRow[j] = tempcell.StringCellValue;
                                            } 
                                        }
            ...
            try
            {
                ...
                for (i = 0; i < data.Rows.Count; ++i)
                {
                    ...
                    for (j = 0; j < data.Columns.Count; ++j)
                    {
                        ...
                        {
                            ////判断类型为Int32
                            if (data.Columns[j].DataType.ToString() == "System.Int32")
                            {
                ...
                if (sheet != null)
                {
                    ...
                    if (isFirstRowColumn)
                    {
                        for (int i = firstRow.FirstCellNum; i < cellCount; ++i)
                        {
                            ...
                            if (cell != null)
                            {
                                string cellValue = cell.StringCellValue==""?"F"+(i+1):cell.StringCellValue;
                                if (cellValue != null)
                                {
                                    DataColumn column = new DataColumn(cellValue);
                                    data.Columns.Add(column);
