    ...
    RemoteImage.prototype = {
        ...
        setImage: function (img) {
            ...
            if(src) {
                ...
                if (w) {
                    $G("width").value = img.style.width || '';
                    $G("height").value = img.style.height || '';
                }
                else {
                ...
                $G("temp-src").value = img.getAttribute("temp-src") || ""
                setAlign(align);
                this.setPreview();
                this.updateLocker();
            }
        ...
        getInsertList: function () {
            ...
            if(data['url']) {
                return [{
                    ...
                    "temp-src": data['wxurl'],
                    _src: data['url'],
                    width: data['width'] || '100%', //data['width'] || '', //100 默认100%
                    height: data['height'] || '',
                    border: data['border'] || '',
                    ...
                    title: data['title'] || '',
                    alt: data['title'] || '',
                    style: "width:" + data['width'] + ";height:" + data['height'] + "" //移除
                }];
            } else {
                return [];
            }
    ...
    UploadImage.prototype = {
        ...
        getInsertList: function () {
            ...
            for (i = 0; i < this.imageList.length; i++) {
                ...
                console.info('getInsertList:' + data);
                list.push({
                    src: prefix + data.url,
                    _src: prefix + data.url,
                    'temp-src': prefix + data.wxurl,
                    title: data.title,
                    alt: data.original,
                    floatStyle: align,
                    style:"width:100%"
        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    console.info('pushData:' + list[i]);
                    img.setAttribute('temp-src', urlPrefix + list[i].wxurl);
        ...
        getInsertList: function () {
            ...
            for (i = 0; i < lis.length; i++) {
                if (domUtils.hasClass(lis[i], 'selected')) {
                    ...
                    wxurl = img.getAttribute('temp-src');
                    console.info('getInsertList:' + img);
                    list.push({
                        src: src,
                        _src: src,
                        'temp-src': wxurl,
