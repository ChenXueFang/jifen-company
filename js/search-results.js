...
param1.PositionKeyWordsString = request("keywords");
// param1.WorkCityString[0] = request("keywords");
...
if (param1.PositionKeyWordsString != '') {
    $('.right_content').prepend('<div class="search-info"><h2><span>搜索</span><span>"' + param1.PositionKeyWordsString + '"</span></h2></div>')
}

function collapse(e) {
    if (e.children[0].children[0].children[0].className.indexOf('icon-up-arrow')>=0) {
...
function searchPosition() {
    // if($('.search_text').val()==''){
    //     param1.PositionTitleString=''
    // }else{
    //     param1.PositionTitleString="Standardization";
    // }
    // param1.PositionKeyWordsString = $('.search_text').val()
    let param = {
        // positionTitlelike: param1.positionTitlelike,
        PositionTitleString:  $('.search_text').val(),
        PositionKeyWordsString:  param1.PositionKeyWordsString,
        jobType: jobType,
...
$('.innerlabel ul').on('click', 'input', function () {
    ...
    if (this.checked) {
        ...
        $(".keyword ul.conditions").append("<li class='" + this.id + "'><span>" + this.parentElement.children[3].innerText + "</span><a href='javascript:;'><i class='icon icon-cancel'></i></a></li>")
    } else {
        switch (this.parentElement.parentElement.parentElement.parentElement.className.split(' innerlabel')[1]) {
            case '1':
    ...
    if (checkedArr.length > 0) {
        $(".keyword").show();
        $(".keyword ul li.clear").show();
    } else {
        $(".keyword").hide();
        $(".keyword ul li.clear").hide();
    }
    searchPosition()
})
$('.keyword ul').on('click', 'li.clear', function () {
    for(var i=checkedArr.length-1;i>=0;i--){
        $('.innerlabel ul #' + checkedArr[i]).click();
    }
})
...
$('.keyword ul.conditions').on('click', '.icon-cancel', function () {
    $('.innerlabel ul #' + this.parentElement.parentElement.className).click();
})
function search() {
...
function save(PositionId, e, jobType, collectionId) {
    ...
    if (e.children[0].className.indexOf('hide-1')>=0) {
...
function collectPosition(positionId, jobType, e) {
    let data = {
    ...
    $.ajax({
        ...
        success: function (res) {
            // searchPosition()
            e.children[0].classList.remove("hide-1");
            e.children[0].classList.add("show-1");
            e.children[1].classList.remove("show-1");
            e.children[1].classList.add("hide-1");
            getCollectionList(jobType);
            $(e).removeAttr("onclick").attr("onclick","save('"+positionId+"',this,\""+jobType+"\",'"+res.rows[0].CollectionId+"')")
        },
        error: function (errors) {
